...
try:
    from repo_diff_init_specialized_fixed_v3 import compare_repositories
...
class RepositoryComparisonApp:
    ...
    def __init__(self, root):
        ...
        default_output = os.path.join(os.path.expanduser("~"), "Desktop", "diff")
        ...
        try:
            ...
            if mode == "specialized" and SPECIALIZED_IMPORT_SUCCESS:
                ...
                script_name = {
                    ...
                    "specialized": "repo_diff_init_specialized_fixed_v3.py"
                ...
                files_compared = files_with_diff = files_only_in_past = files_only_in_original = -1
            ...
            self.log_message(f"Created {len(created_files)} files in output directory")
            
            # Update UI after comparison is complete
            self.root.after(0, self.comparison_complete, success, output_path, 
                           files_compared, files_with_diff, files_only_in_past, files_only_in_original)
    ...
    def comparison_complete(self, success, output_path, files_compared, files_with_diff, files_only_in_past, files_only_in_original):
        ...
        if success:
            ...
            if files_compared >= 0:
                ...
                self.log_message(f"Files only in original repository: {files_only_in_original}")
                self.log_message(f"Total differences: {files_with_diff + files_only_in_past + files_only_in_original}")
            ...
            if created_files:
                self.log_message(f"Successfully created {len(created_files)} files in the output directory.")
                ...
                self.log_message("WARNING: No files were created in the output directory.")
                messagebox.showwarning(
                    "Comparison Complete", 
                    f"Repository comparison completed, but no files were created in the output directory.\n"
                    f"This could mean there were no differences, or there was an issue with file creation."
                )
        else:
            self.log_message("-----------------------------------")
